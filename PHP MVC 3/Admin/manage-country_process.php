...
$query = mysqli_query($conn, "select n.id,n.Name,n.Country_code,n.CreatedDate,u.FirstName,u.LastName,n.IsActive from countries n inner join users u on n.CreatedBy = u.id where  `u`.`IsActive` = '1' LIMIT $start, $limit");
...
<thead>
    ...
    <th scope="col" class="table-column-header">country name</th>
    <th scope="col" class="table-column-header">country code</th>
...
if($_POST['query'] != '')
{
    $query = mysqli_query($conn, "select n.id,n.Name,n.Country_code,n.CreatedDate,u.FirstName,u.LastName from countries n inner join users u on n.CreatedBy = u.id where  `u`.`IsActive` = '1' AND u.Firstname LIKE '%$search%' or n.Name LIKE '%$search%' or n.Description LIKE '%$search%' or n.CreatedDate LIKE '%$search%' LIMIT $start, $limit");
 
  $query1 = mysqli_query($conn, "select n.id,n.Name,n.Country_code,n.CreatedDate,u.FirstName,u.LastName from countries n inner join users u on n.CreatedBy = u.id where  `u`.`IsActive` = '1' AND u.Firstname LIKE '%$search%' or n.Name LIKE '%$search%' or n.Description LIKE '%$search%' or n.CreatedDate LIKE '%$search%'");
  ...
  if (mysqli_num_rows($query) > 0) {
    while ($row = mysqli_fetch_array($query)) {
        ...
        <td>' . $active . '</td>
        <td> 
		<a href="add-country.php?id='.$catid.'"><img src="images/images/edit.png" alt=""></a>
    &nbsp;
          ...
          <div class="modal-header">
             <div class="container">
                 <div class="content-box">
                   <h4>Are you sure you want to make this Country Inactive ?</h4>
                 </div>
          </div>
