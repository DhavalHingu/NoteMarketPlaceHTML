...
$query = mysqli_query($conn, "select s.id , s.Title , s.CreatedDate , s.status , n.Name , u.FirstName , u.id as us,r.Value from `seller_notes` s INNER join `notecategories` n on s.Category = n.id inner join `referencedata` r on s.status = r.id inner join users u on s.seller_id = u.id where `status` != '9' AND `s`.`IsActive` = '1' LIMIT $start, $limit");
$output = '';
...
if($_POST['query'] != '')
{
  $query = mysqli_query($conn, "select s.id , s.Title , s.CreatedDate , s.status , n.Name , u.FirstName ,u.id as us, r.Value from `seller_notes` s INNER join `notecategories` n on s.Category = n.id inner join `referencedata` r on s.status = r.id  inner join users u on s.seller_id = u.id where `status` != '9' AND `s`.`IsActive` = '1'  AND `s`.`Title` like '%$search%' OR n.Name LIKE '%$search%' OR r.Value LIKE '%$search%' OR s.CreatedDate LIKE '%$search%' OR u.FirstName LIKE '%$search%' LIMIT $start , $limit");

  $query1 = mysqli_query($conn, "select s.id , s.Title , s.CreatedDate , s.status , n.Name , u.FirstName ,u.id as us, r.Value from `seller_notes` s INNER join `notecategories` n on s.Category = n.id inner join `referencedata` r on s.status = r.id  inner join users u on s.seller_id = u.id where `status` != '9' AND `s`.`IsActive` = '1'  AND `s`.`Title` like '%$search%' OR n.Name LIKE '%$search%' OR r.Value LIKE '%$search%' OR s.CreatedDate LIKE '%$search%' OR u.FirstName LIKE '%$search%'");
...
if($_POST['query2'] != '')
{
  ...
  $query = mysqli_query($conn, "select s.id , s.Title , s.CreatedDate , s.status , n.Name , u.FirstName ,u.id as us, r.Value from `seller_notes` s INNER join `notecategories` n on s.Category = n.id inner join `referencedata` r on s.status = r.id  inner join users u on s.seller_id = u.id where `status` != '9' AND `s`.`IsActive` = '1'  AND `u`.`FirstName` like '%$search1%' group by seller_id LIMIT $start , $limit");
  ...
  if (mysqli_num_rows($query) > 0) {
    while ($row = mysqli_fetch_array($query)) {
        ...
        <td>' . $row['Name'] . '</td>
        <td>' . $row['FirstName']. '&nbsp <a href="member-details.php?id='.$userid.'"><img src="../images/images/eye.png" alt=""></a>'. '</td>
        <td>' . $row['CreatedDate'] . '</td>
        <td>' . $row['Value'] . '</td>
        <td class="" style="width:32%">
		<a class="btn btn-success" href="#" data-toggle="modal" data-target="#exampleModal" onClick="showModal('.$noteid.')">Approve</a>
        &nbsp;
    <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#exampleModal3" onClick="showModal('.$noteid.')">Reject</button>	
     &nbsp;
     <button type="button" class="btn btn-light" data-toggle="modal" data-target="#exampleModal2" onClick="showModal('.$noteid.')">In Review</button>
      &nbsp;
