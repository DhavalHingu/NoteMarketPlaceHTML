...
//count the records
  $result_count = mysqli_query($conn,"select COUNT(*) as total_records from `downloads` where `downloads`.`Seller` = '$seller_id'");
...
$phone="";
$paidornot="";
$selectcategory = mysqli_query($conn,"Select * from downloads,notecategories where downloads.Note_category = notecategories.id and downloads.Seller = '$seller_id' ");


$search = $_POST['query'];
//if page not set then displays all the data
$query = mysqli_query($conn, "select d.id,d.note_id,d.Allowed_Download,d.Note_title,d.Purchased_price,d.CreatedDate,n.Name,u.Email_id,u.FirstName,us.Phone_number,r.Value from downloads d INNER JOIN users u ON d.Downloader = u.id AND d.Seller = '$seller_id' INNER JOIN  user_profile us ON d.Downloader = us.user_id AND d.Seller = '$seller_id'  INNER JOIN notecategories n ON d.Note_category = n.id AND d.Seller = '$seller_id' INNER JOIN referencedata r on d.IsPaid = r.id AND d.Seller = '$seller_id'  WHERE d.Seller ='$seller_id' LIMIT $start , $limit ");
$output = '';
...
if($_POST['query'] != '')
{
  $query = mysqli_query($conn, "select d.id,d.note_id,d.Allowed_Download,d.Note_title,d.Purchased_price,d.CreatedDate,n.Name,u.Email_id,u.FirstName,us.Phone_number,r.Value from downloads d inner JOIN users u ON d.Downloader = u.id AND d.Seller = '$seller_id' inner JOIN  user_profile us ON d.Downloader = us.user_id AND d.Seller = '$seller_id'  inner JOIN notecategories n ON d.Note_category = n.id AND d.Seller = '$seller_id' inner JOIN referencedata r on d.IsPaid = r.id AND d.Seller = '$seller_id'  WHERE d.Seller ='$seller_id' AND  `Note_title` LIKE '%$search%'  OR `Name` LIKE '%$search%' OR `Email_id` LIKE '%$search%'  OR `Phone_number` LIKE '%$search% '  OR `Name` LIKE '%$search%'  OR `Purchased_price` LIKE '%$search%'  OR `Attachment_downloaded_date` LIKE '%$search%' LIMIT $start , $limit");

  $query1 = mysqli_query($conn, "select d.id,d.note_id,d.Allowed_Download,d.Note_title,d.Purchased_price,d.CreatedDate,n.Name,u.Email_id,u.FirstName,us.Phone_number,r.Value from downloads d inner JOIN users u ON d.Downloader = u.id AND d.Seller = '$seller_id' inner JOIN  user_profile us ON d.Downloader = us.user_id AND d.Seller = '$seller_id'  inner JOIN notecategories n ON d.Note_category = n.id AND d.Seller = '$seller_id' inner JOIN referencedata r on d.IsPaid = r.id AND d.Seller = '$seller_id' WHERE d.Seller ='$seller_id' AND  `Note_title` LIKE '%$search%'  OR `Name` LIKE '%$search%' OR `Email_id` LIKE '%$search%'  OR `Phone_number` LIKE '%$search% '  OR `Name` LIKE '%$search%'  OR `Purchased_price` LIKE '%$search%'  OR `Attachment_downloaded_date` LIKE '%$search%' ");
...
<a href="note-details.php?id='.$row['id'].'"><img src="images/images/eye.png" alt=""></a> &nbsp;
